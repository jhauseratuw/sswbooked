...
require_once(ROOT_DIR . 'lib/Email/Messages/AccountApprovedEmail.php');
...
class ManageUsersPresenter extends ActionPresenter implements IManageUsersPresenter
{
    ...
    public function Activate()
    {
        ...
        // Check database to see if activation email has been sent
		$userId = $user->Id();
        $activationEmailCommand = new GetActivationEmailSent($userId);
        $reader = ServiceLocator::GetDatabase()->Query($activationEmailCommand);
        $row = $reader->GetRow();

        // Only send the email if it hasn't been sent
        if (! $row['activation_email_sent']) {
            ServiceLocator::GetEmailService()->Send(new AccountApprovedEmail($user));
            
            // Update the activation_email_sent field
            $activationEmailCommand = new UpdateActivationEmailSent($userId, 1);
            $reader = ServiceLocator::GetDatabase()->Query($activationEmailCommand);
        }
    }
