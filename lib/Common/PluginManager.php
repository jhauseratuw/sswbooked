...
class PluginManager
{
	...
	public function LoadAuthentication()
	{
		...
		// TODO: testing
		// Validate using Shibboleth if user has a valid CSRF token
		$has_token = !empty($_POST['token']);		
		if ($has_token) {
		   $is_token_valid = hash_equals($_SESSION['token'], $_POST['token']);
		   if ($is_token_valid) {
			   require_once('../plugins/Authentication/Shibboleth/Shibboleth.php');
			   $plugin = new Shibboleth($authentication);
			   return $plugin;
		   }
	   	}
