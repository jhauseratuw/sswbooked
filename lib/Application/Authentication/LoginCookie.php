...
class LoginCookie extends Cookie
{
	...
	public function __construct($userid, $lastLoginTime)
	{
		...
		// Set cookie expiration to one day
		$expiration = Date::Now()->AddDays(1)->TimeStamp();

		parent::__construct(CookieKeys::PERSIST_LOGIN, sprintf('%s|%s', $userid, $lastLoginTime), $expiration);
	}
