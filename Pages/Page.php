...
abstract class Page implements IPage
{
	...
	protected function __construct($titleKey = '', $pageDepth = 0)
	{
		...
		$this->smarty->assign('LoginName', $this->GetLoginName());
	...
	{
		if ($_SESSION['login_method'] == 'normal') {
			$userSession = ServiceLocator::GetServer()->GetUserSession();
			$email = $userSession->Email;
			$login_name = preg_replace('/\@.*/', '', $email);
			return $login_name;
		}
	...
	public function IsAuthenticated()
	{
		...
		$user_session = $this->server->GetUserSession();
		$valid_user_session = !is_null($user_session) && $user_session->IsLoggedIn();
		if ($valid_user_session) {
			// Only check if the user logged in with netid
			if ($_SESSION['login_method'] != 'shibboleth') {
				return true;
			}

			$netid = $_SERVER['REMOTE_USER'];
			$user_netid = $user_session->Username;
			// Check that logged in netid is equal to user netid
			if ($netid === $user_netid) {
				return true;
			} else {
				$redirect = Configuration::Instance()->GetSectionKey('development', 'netid');
				header('location: /psych/department/admin/PsyCal/Web/logout.php');
				die();
			}
		} else {
			return false;
